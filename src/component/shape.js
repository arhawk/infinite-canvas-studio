...
function getShapeData(node, overrides = {}) {
  ...
  return {
    ...
    height: normalizeDimension(overrides.height, node.height?.() ?? DEFAULT_HEIGHT, MIN_HEIGHT),
...
function syncShapeVisuals(node, overrides = {}) {
  ...
  const hitArea = node.findOne(".shape-hit-area");
...
export function applyShapeStyle(node, style = {}) {
  ...
  return syncShapeVisuals(node, {
    ...
    textColor: normalizeColor(style.textColor, node.getAttr("shapeTextColor") ?? DEFAULT_TEXT_COLOR),
    fontSize: clampNumber(
...
function attachShapeInlineEditor(group) {
  const openInlineEditor = (event = {}) => {
    ...
    const anchorNode = Array.from(group.find(".shape-visual"))
      .find((node) => node.visible?.() !== false) ?? group;
    const groupBox = anchorNode.getClientRect({
      relativeTo: stage,
      skipShadow: true,
      skipStroke: true,
    });
  ...
  group.on("dblclick.shapeInlineEditor dbltap.shapeInlineEditor", openInlineEditor);
  group.find(".shape-visual").forEach((visual) => {
    visual.off(".shapeInlineEditor");
    visual.on("dblclick.shapeInlineEditor dbltap.shapeInlineEditor", openInlineEditor);
  });
}
...
export class ShapeComponent extends BaseComponent {
  ...
  async createNode({
    ...
    group.add(
      new Konva.Rect({ name: "shape-hit-area" }),
  ...
  renderPalettePreview(container) {
    ...
    container.innerHTML = `
      ...
      <span class="shape-component__preview-diamond"></span>
    `;
  }
