...
- Toolbar helper controls are contextual: `arrange` shows focus actions only when a focusable node is selected, while `brush` shows color and stroke controls only when that tool is active.
